...
L02AB:  DEC     L               ; cycles 2F>2E>2D>2C>2B>2A>29>28 for
 ...
 ;       RET     Z              ; return if one key or no key.
        JR      Z,UNMRKSHFT     ; return if one key or no key.
 
        CP      $28             ; is it capsshift (was $27) ?
        JR      Z,MRKSHFT       ; return if so.

        CP      $19             ; is it symbol shift (was $18) ?
        JR      Z,UNMRKSHFT     ; return also

        LD      A,E             ; now test E
        LD      E,D             ; but first switch
        LD      D,A             ; the two keys.
        CP      $18             ; is it symbol shift ?
        RES     4,(IY+$01)      ; return if so.
        RET                     ; return (with zero set if it was).
                                ; but with symbol shift now in D

MRKSHFT:   SET 4,(IY+$01)       ; Set unused flag to SET for shift  
        RET

UNMRKSHFT:   RES 4,(IY+$01)
        RET
; ----------------------
...
;   now the raw key (0-38d) is converted to a main key (uppercase).
        ...
        ;JR      NC,MRKSHFT      ; return if just a single shift
...
L034A:  LD      D,$00           ; prepare to index.
        ...
        CP      $0C             ; Check if DELETE to swap with Shift-Space
        JR      Z,SWAPDEL       ;    
        RET                     ; return.
...
;; BREAK-KEY
L1F54:  LD      A,$EF           ; Input address: $EFFE ; Panagis Upper right row
        IN      A,($FE)         ; read upper right keys
        RRA                     ; rotate bit 0 - SPACE ; Panagis 0 is pressed
